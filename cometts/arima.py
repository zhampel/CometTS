...
# Functions for an auto-regressive integrated moving average
# analysis using CometTS on a time series of satellite imagery.

DEFAULT_TITLE_LABEL = 'Brightness over time - '\
                      'Census Tract 9509, Yabucoa Municipio'


def run_plot_TS(main_gdf, figsize=(12, 6), y_val_alpha=1, scatter_alpha=1,
                error_alpha=0.2, y_label="Brightness", x_label="Date",
                title_label=DEFAULT_TITLE_LABEL,
                outname="/Outname.csv", figname='', custom_x_axis=True,
                show_grid=True, show_legend=True,
                min_count=0.5, ymin=0, ymax=5000):
    print("Plotting...")
    C = 0
    for item in main_gdf['ID'].unique():
        ...
        if 'SeasonalForecast' in gdf3.columns:
            ...
            if len(xdate) == len(gdf3['Trend']):
                ...
                # ax.plot(xdate, RM, '-',label="Moving Median",
                #         alpha=y_val_alpha)

                # Seasonal Forecast
                T = interpolate_gaps(T, limit=3)
                ax.plot(xdate, T, '-', label="Seasonal Forecast",
                        alpha=1, color='#FF8700')

                # scatter points-median top, mean bottom
                ax.scatter(xdate, y, label="Mean", s=50,
                ...
                if 'observations' in gdf3.columns:
                    ax.scatter(xdate, count, label="# of obs",
                               s=50, color='#330DD0',
                               alpha=y_val_alpha, marker='d')
                ...
                if custom_x_axis:
                    ...
                    # ax.set_xlim(
                    # datetime.date(settings.plot['x_min'], 1, 1),
